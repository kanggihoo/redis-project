...
`@MockitoBean` replaces the deprecated `@MockBean` annotation in Spring Boot 4.0+. It is now part of the core **Spring Framework (`spring-test`)** rather than being Spring Boot-specific. It utilizes the new **Bean Overriding API** to register a Mockito mock in the Spring context, replacing any existing bean of the same type.

**Import Path:** `org.springframework.test.context.bean.override.mockito.MockitoBean`
...
Spring's Bean Overriding infrastructure handles the lifecycle of these mocks automatically. **Mocks are reset between internal test methods** to ensure isolation.

Manual resetting is generally discouraged, but can be done if absolutely necessary within a single test:
...
### After (Spring Boot 4+ / Spring Framework 7+)
